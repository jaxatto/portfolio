...
const iconNamesMatch = iconNamesContent.match(
	/export const iconNames = \[(.*?)\] as const;/s,
);
...
const importLines = svgFiles
	.map(
		(name) => `import ${toPascalCase(name)} from '@/assets/icons/${name}.svg';`,
	)
	.join('\n');
...
const importInsertMatch =
	updatedIconComponentContent.match(/^(import .*?;\n)+/);
...
fs.writeFileSync(
	iconComponentFile,
	updatedIconComponentContent.trim() + '\n',
	'utf8',
);
